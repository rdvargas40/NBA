...
def create_table():
    """ Creates the players_matchs table """
    exec_sql_query("""--sql
        CREATE TABLE IF NOT EXISTS players_matchs (
...
def populate_database() -> List:
    """
    Populate the database with the players matchs files hosted in S3
    Returns:
    ...
    s3_pages = s3_paginator.paginate(Bucket='nba.pipeline', Prefix="players/matchs")
    s3_file_paths = []
