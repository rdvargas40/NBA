...
```bash
trans-api$ sam build --use-container
```
...
```bash
trans-api$ sam logs -n StockCheckerFunction --stack-name trans-api --tail
```
...
```bash
trans-api$ pip install pytest pytest-mock --user
trans-api$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name trans-api
```
